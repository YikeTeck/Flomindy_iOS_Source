...
if (freq < 6) return 'Delta/Theta';
if (freq < 9) return 'Theta';
if (freq < 12) return 'Alpha (Low)';
if (freq < 15) return 'Alpha (High)';
return 'Beta';
};
// Ottieni la descrizione in base alla frequenza
const getBrainWaveDescription = (freq) => {
if (freq < 6) return 'Sleep & Relaxation';
if (freq < 9) return 'Deep Relaxation';
if (freq < 12) return 'Deep Learning';
if (freq < 15) return 'Dynamic Learning';
...
Alert.alert(
"Background Mode",
`Flomindy funzionerà in background con frequenza EEG di ${frequency.toFixed(1)} Hz.`,
[{ text: "OK" }]
);
// Lascia un commento per futura implementazione
console.log(`Background mode attivato - frequenza: ${frequency.toFixed(1)} Hz`);
};
...
<View style={styles.headerContainer}>
  <TouchableOpacity style={styles.menuButton} onPress={() => alert("Menu funzionante!")}>
    <Text style={styles.menuIcon}>☰</Text>
  </TouchableOpacity>
  <View style={styles.titleContainer}>
    <Text style={{...styles.title, fontSize: 24, marginBottom: 4}}>Flomindy</Text>
    <Text style={{...styles.subtitle, fontSize: 14, marginBottom: 10}}>Brain Stimulation App</Text>
  </View>
</View>
<View style={{...styles.frequencyContainer, padding: 12, marginBottom: 18}}>
<View style={styles.frequencyHeader}>
<Text style={styles.frequencyLabel}>Frequency EEG</Text>
...
<Text style={styles.controlLabel}>Deep Effect</Text>
...
<View style={{
position: 'absolute',
bottom: 15, // Aumentato da 10 a 15 per abbassare leggermente i pulsanti
left: 0,
right: 0,
alignItems: 'center',
paddingHorizontal: 20,
...
{isActive ? 'Focusing Now' : 'Ready to Focus'}
</Text>
</View>
</View>
</SafeAreaView>
